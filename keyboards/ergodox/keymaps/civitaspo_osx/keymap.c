#include "ergodox.h"
#include "debug.h"
#include "action_layer.h"

#define L_BASE 0 // default layer
#define L_SHFT 1 // shift layer
#define L_MISC 2 // misc layer
#define L_MDIA 3 // media keys

// Fillers to make layering clearer
#define _______ KC_TRNS
#define XXXXXXX KC_NO

// Custom Aliases
#define G(kc) LGUI(kc)
#define A(kc) LALT(kc)

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
/* Keymap 0: Basic layer
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * |  ESC   |   [  |   {  |   }  |   (  |   =  |      |           |  \   |   *  |   )  |   +  |   ]  |   !  |   $    |
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * |  TAB   |   '  |   ,  |   .  |   p  |   y  |      |           |  @   |   f  |   g  |   c  |   r  |   l  |   /    |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * | LCtrl  |   a  |   o  |   e  |   u  |   i  |------|           |------|   d  |   h  |   t  |   n  |   s  |   -    |
 * |--------+------+------+------+------+------|      |           |  #   |------+------+------+------+------+--------|
 * | L_SHFT |   ;  |   q  |   j  |   k  |   x  |      |           |      |   b  |   m  |   w  |   v  |   z  | RShift |
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   |LGui+C|LGui+V|      | Alt  | LGui |                                       |L_MISC|      |      |      |      |
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        |      |      |       |      |  &   |
 *                                 ,------|------|------|       |------+------+------.
 *                                 |      |      |      |       |L_MDIA|      |      |
 *                                 | Space|Back  |------|       |------|Back  |Enter |
 *                                 |      |space |AltSPC|       |LG+SPC|space |      |
 *                                 `--------------------'       `--------------------'
 */
// If it accepts an argument (i.e, is a function), it doesn't need KC_.
// Otherwise, it needs KC_*
[L_BASE] = KEYMAP(  // layer 0 : default
        // left hand
        KC_ESC,     KC_LBRC, KC_LCBR, KC_RCBR, KC_LPRN, KC_EQL, _______,
        KC_TAB,     KC_QUOT, KC_COMM, KC_DOT,  KC_P,    KC_Y,   _______,
        KC_LCTL,    KC_A,    KC_O,    KC_E,    KC_U,    KC_I,
        MO(L_SHFT), KC_SCLN, KC_Q,    KC_J,    KC_K,    KC_X,   _______,
        G(KC_C),    G(KC_V), _______, KC_LALT, KC_LGUI,
                                                       _______, _______,
                                                                _______,
                                               KC_SPC, KC_BSPC, A(KC_SPC),
        // right hand
        KC_BSLS, KC_ASTR, KC_RPRN,    KC_PLUS, KC_RBRC, KC_EXLM, KC_DLR,
        KC_AT,   KC_F,    KC_G,       KC_C,    KC_R,    KC_L,    KC_SLSH,
                 KC_D,    KC_H,       KC_T,    KC_N,    KC_S,    KC_MINS,
        KC_HASH, KC_B,    KC_M,       KC_W,    KC_V,    KC_Z,    KC_RSFT,
                          MO(L_MISC), _______, _______, _______, _______,
        _______,    KC_AMPR,
        TG(L_MDIA),
        G(KC_SPC),  KC_BSPC, KC_ENT
    ),
/* Keymap 1: Shift layer
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * |        |   7  |   5  |   3  |   1  |   9  |      |           |  |   |   0  |   2  |   4  |   6  |   8  |   ~    |
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * |        |   "  |   ,  |   .  |   P  |   Y  |      |           |  ^   |   F  |   G  |   C  |   R  |   L  |   ?    |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * | LCtrl  |   A  |   O  |   E  |   U  |   I  |------|           |------|   D  |   H  |   T  |   N  |   S  |   _    |
 * |--------+------+------+------+------+------|      |           |  `   |------+------+------+------+------+--------|
 * |        |   :  |   Q  |   J  |   K  |   X  |      |           |      |   B  |   M  |   W  |   V  |   Z  |        |
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   |      |      |      |AltSft|LG+Sft|                                       |      |      |      |      |      |
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        |      |      |       |      |  %   |
 *                                 ,------|------|------|       |------+------+------.
 *                                 |      |      |      |       |      |      |      |
 *                                 |      |      |------|       |------|      |      |
 *                                 |      |      |      |       |      |      |      |
 *                                 `--------------------'       `--------------------'
 */
// NUMBER AND SHIFT
[L_SHFT] = KEYMAP(
       // left hand
       _______,    KC_7,       KC_5,       KC_3,       KC_1,       KC_9,    _______,
       _______,    S(KC_QUOT), S(KC_COMM), S(KC_DOT),  S(KC_P),    S(KC_Y), _______,
       S(KC_LCTL), S(KC_A),    S(KC_O),    S(KC_E),    S(KC_U),    S(KC_I),
       _______,    S(KC_SCLN), S(KC_Q),    S(KC_J),    S(KC_K),    S(KC_X), _______,
       _______,    _______,    _______,    S(KC_LALT), S(KC_LGUI),
                                                                   _______, _______,
                                                                            _______,
                                                          _______, _______, _______,
       // right hand
       KC_PIPE, KC_0,    KC_2,    KC_4,    KC_6,    KC_8,    KC_TILD,
       KC_CIRC, S(KC_F), S(KC_G), S(KC_C), S(KC_R), S(KC_L), KC_QUES,
                S(KC_D), S(KC_H), S(KC_T), S(KC_N), S(KC_S), KC_UNDS,
       KC_GRV,  S(KC_B), S(KC_M), S(KC_W), S(KC_V), S(KC_Z), _______,
                          _______, _______, _______, _______, _______,
       _______, KC_PERC,
       _______,
       _______, _______, S(KC_ENT)
),
/* Keymap 2: Misc Layer
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * |        |  F7  |  F5  |  F3  |  F1  |  F9  |      |           |      |  F10 |  F2  |  F4  |  F6  |  F8  |   F11  |
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * |        |      | Rclk | MsUp | Lclk |      |      |           |      |      |      |  Up  |      |      |   F12  |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |        |      |MsLeft|MsDown|MsRght|      |------|           |------|      | Left | Down |Right |      |        |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * | LShift |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   |      |      |      |      |      |                                       |      |      |      |      |      |
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        |      |      |       |      |      |
 *                                 ,------|------|------|       |------+------+------.
 *                                 |      |      |      |       |      |      |      |
 *                                 |      |      |------|       |------|      |      |
 *                                 |      |      |      |       |      |      |      |
 *                                 `--------------------'       `--------------------'
 */
// SYMBOLS
[L_MISC] = KEYMAP(
       // left hand
       _______, KC_F7,   KC_F5,   KC_F3,   KC_F1,   KC_F9,   _______,
       _______, _______, KC_BTN2, KC_MS_U, KC_BTN1, _______, _______,
       _______, _______, KC_MS_L, KC_MS_D, KC_MS_R, _______,
       KC_LSFT, _______, _______, _______, _______, _______, _______,
       _______, _______, _______, _______, _______,
                                                    _______, _______,
                                                             _______,
                                           _______, _______, _______,
       // right hand
       _______, KC_F10,    KC_F2, KC_F4,   KC_F6,    KC_F8,   KC_F11,
       _______, _______, _______, KC_UP,   _______,  _______, KC_F12,
                _______, KC_LEFT, KC_DOWN, KC_RIGHT, _______, _______,
       _______, _______, _______, _______, _______,  _______, _______,
                         _______, _______, _______,  _______, _______,
       _______, _______,
       _______,
       _______, _______, _______
),
/* Keymap 3: Media and mouse keys
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * |  RESET |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * |        |      |      |      |      |      |      |           |      |      |      |      |      |      |        |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |        |      |      |      |      |      |------|           |------|      |      |      |      |      |  Play  |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * |        |      |      |      |      |      |      |           |      |      |      | Prev | Next |      |        |
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   |      |      |      |      |      |                                       |VolUp |VolDn | Mute |      |      |
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        |      |      |       |      |      |
 *                                 ,------|------|------|       |------+------+------.
 *                                 |      |      |      |       |      |      |Brwser|
 *                                 |      |      |------|       |------|      |Back  |
 *                                 |      |      |      |       |      |      |      |
 *                                 `--------------------'       `--------------------'
 */
// MEDIA AND MOUSE
[L_MDIA] = KEYMAP(
       // left hand
       RESET,   _______, _______, _______, _______, _______, _______,
       _______, _______, _______, _______, _______, _______, _______,
       _______, _______, _______, _______, _______, _______,
       _______, _______, _______, _______, _______, _______, _______,
       _______, _______, _______, _______, _______,
                                           _______, _______,
                                                    _______,
                                  _______, _______, _______,
       // right hand
       _______,    _______, _______, _______, _______, _______, _______,
       _______,  _______, _______, _______, _______, _______, _______,
                 _______, _______, _______, _______, _______, KC_MPLY,
       _______,  _______, _______, KC_MPRV, KC_MNXT, _______, _______,
                          KC_VOLU, KC_VOLD, KC_MUTE, _______, _______,
       _______, _______,
       _______,
       _______, _______, KC_WBAK
),
};

const uint16_t PROGMEM fn_actions[] = {
    // [1] = ACTION_LAYER_TAP_TOGGLE(L_MISC)                // FN1 - Momentary Layer 1 (Symbols)
};

const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
    // MACRODOWN only works in this function
    // switch(id) {
    //   case 0:
    //   if (record->event.pressed) {
    //     register_code(KC_RSFT);
    //   } else {
    //     unregister_code(KC_RSFT);
    //   }
    //   break;
    // }
    return MACRO_NONE;
};

// Runs just one time when the keyboard initializes.
void matrix_init_user(void) {

};

// Runs constantly in the background, in a loop.
void matrix_scan_user(void) {

    /*
     * NOTE: My ErgoDox does not have LED,
     *       so does not need the below actions.
     *
    uint8_t layer = biton32(layer_state);

    ergodox_board_led_off();
    ergodox_right_led_1_off();
    ergodox_right_led_2_off();
    ergodox_right_led_3_off();
    switch (layer) {
        case 1:
            ergodox_right_led_1_on();
            break;
        case 2:
            ergodox_right_led_2_on();
            break;
        case 3:
            ergodox_right_led_1_on();
            ergodox_right_led_2_on();
            break;
        default:
            // none
            break;
    }
    */
};
